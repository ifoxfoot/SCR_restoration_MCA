...
#bind to unbuffered forests dataframe
forests_flammable <- merge(bios_veg_layer_forest, extract_flammable_buffered, by= "ID")
```
...
tm_shape(extract_flammable_buffered_sf) +
  tm_fill("layer", title = "Tree Species", palette = "Spectral") +
  tm_view(set.view = 12)
```
...
```{r}
#st_write(fire_risk_classed, here("shapefiles_written", "fire_risk.shp"))
```
